...
class HistoricalDataFeed:
    ...
    @staticmethod
    def _format_time_param(dt: Optional[datetime], timeframe: str) -> Optional[str]:
        """
        Format times for Alpaca API:
        - Daily frames ('1Day') use date-only: YYYY-MM-DD
        - Intraday frames use RFC3339 UTC: YYYY-MM-DDTHH:MM:SSZ
        """
        if dt is None:
            return None
        if timeframe == "1Day":
            return dt.strftime("%Y-%m-%d")
        # Ensure timezone-aware UTC and format with trailing 'Z'
    ...
    def get_bars(
        ...
        """
        try:
            # RFC3339/date formatting for Alpaca API
            start_str = self._format_time_param(start, timeframe)
            end_str = self._format_time_param(end, timeframe)
