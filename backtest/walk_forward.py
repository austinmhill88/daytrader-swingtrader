...
class WalkForwardBacktester:
    ...
    def __init__(self, config_or_path: Union[str, Dict]):
        """
        Initialize walk-forward backtester.

        Args:
            config_or_path: Either a path to configuration YAML (str) or a loaded config dict
        """
        # Accept both a string path and a pre-loaded config dict
        if isinstance(config_or_path, str):
            self.config = load_config(config_or_path)
        elif isinstance(config_or_path, dict):
            self.config = config_or_path
        else:
            raise TypeError("Expected config path (str) or config dict")
        ...
        self.strategies = []
        if self.config.get('strategies', {}).get('intraday_mean_reversion', {}).get('enabled', False):
            self.strategies.append(
                IntradayMeanReversion(self.config['strategies']['intraday_mean_reversion'])
            )

        if self.config.get('strategies', {}).get('swing_trend_following', {}).get('enabled', False):
            self.strategies.append(
...
# Backwards compatibility for existing imports in src/ml_pipeline.py
# Example: from backtest.walk_forward import WalkForwardValidator
WalkForwardValidator = WalkForwardBacktester